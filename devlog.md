...
## 2026-06-26 — Architecture generality: negative result holds for fastText too (queue item)

- Downloaded fastText-300 (~1GB, gensim-data; a genuinely different *architecture* — subword — vs GloVe's count-based co-occurrence) and added it to `run_generality.py`. **Result: blend beats raw in 0 of 6 cells** now spanning two architectures: fastText × WordSim-353 raw 0.718 / blend 0.652 (−0.066); fastText × SimLex-999 raw 0.445 / blend 0.445 (**−0.0000, break-even**). The penalty is regime-dependent — largest on weaker embeddings/relatedness (−0.08), shrinking to break-even on the strongest embedding + hardest similarity task — but **never goes positive**. So the negative result generalises across embedding size, *architecture*, and dataset: similarity-weighted reconstruction is at best break-even and usually a loss on clean vectors. The empirical generality story is complete (within English single-word similarity).
- Updated FINDINGS Result 5 (6-row table + revised conclusion) + the page. Operator unchanged (`blend_from_neighbors`); local script, not CI. Suite **109 passed** (no src change). Drained item 1; next is LT-5 (control-system reframing).

## 2026-06-26 — LT-5: control-system reframing argued, not asserted (queue item)

- Wrote `literature/control-reframing.md` (no code; a grounded, cited position) on Pygmalion's "neural nets are information-flow controllers, not learners" claim. Split it honestly: the **flow-control lens is genuinely grounded** — attention-as-learned-gain (Vaswani 2017), LSTM/GRU gating (Hochreiter & Schmidhuber 1997; Cho 2014), Mixture-of-Experts routing (Shazeer 2017), Highway nets (Srivastava 2015) — but the **strong form is a false dichotomy**: the controllers (attention weights, gates, routers) are themselves *learned* by gradient descent, so "not learners" contradicts how they arise; and the control/learning *separation* Pygmalion proposes is a design idea (cf. hypernetworks/meta-learning), not a result. Verdict: a defensible lens to argue in specific mechanisms (gating/attention), not a fact to assert. Cross-referenced from `REVIEW.md` §3 item 6.
- Drained the Active queue; refilled with one bounded item (phrase composition — LT-4 first slice) and **flagged plainly** that the remaining todo (full topos internal logic, learned memory gate, full linguistic pipeline) are large open research, not hourly-tick work — the natural hand-back-for-scope point once the queue next drains to only those. Suite **109 passed** (no src change).

## 2026-06-26 — Phrase composition (LT-4 first slice) (queue item)

- Built `src/patchi/compose.py`: `compose_phrase(lex, words, method)` — `additive` (Σ vec), `multiplicative` (elementwise Π; the Mitchell & Lapata / Hadamard baselines), and `weighted` (similarity-weighted toward the phrase centroid, **reusing `blend_from_neighbors`** so the weighting is identical code to the blend benchmark). This is the syntax→semantics *composition* step, distinct from the single-word `blend` (reconstruct-from-neighbours).
- Added `tests/test_compose.py` (5 tests): additive = vector sum; multiplicative = elementwise product; weighted down-weights an outlier word (cos-to-cluster 0.998 vs additive 0.934, measured); single-word passthrough; unknown-method/missing-word/empty-phrase raise. Full suite **114 passed**. Next: a phrase-composition benchmark (synthetic, CI-safe; + real M&L dataset if findable) — does weighted composition beat the baselines? After that, the remaining todo is large open research (hand-back point).

## 2026-06-26 — Phrase-composition benchmark + hand-back point reached (queue item)
