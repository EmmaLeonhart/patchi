...
- **G.A. Miller, "WordNet: A Lexical Database for English" (1995), *CACM* 38(11):39–41** — DOI 10.1145/219717.219748. A hand-built graph of synsets linked by hypernymy/hyponymy, synonymy, antonymy, meronymy. → **The independent relation source for `FINDINGS.md` Result 7**: structural similarity is computed over WordNet's hypernym-path ancestors, so the test of Pygmalion's "shared-neighbour distance" claim never reuses the embedding geometry. *Dependency:* `nltk` + the `wordnet`/`omw-1.4` corpora (`python -m nltk.downloader wordnet omw-1.4`), a one-time ~10 MB download used only by `scripts/run_structural.py` and `scripts/run_structural_weighted.py` (the learned-combiner follow-up) — like the GloVe/fastText caches it is **not** part of CI (the `patchi.structural` module itself has no nltk dependency, so the suite stays green offline).
